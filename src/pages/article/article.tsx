...
import {studentFinishTask, studentGetTaskArticle} from "../../api/methods/class.methods.ts";
...
export default function Article() {
  ...
  const { data, error, loading, onSuccess } = useWatcher(() => {
    if (type === 'keep') {
      ...
      const part = article.split(':');
      return studentGetTaskArticle(part[1]);
    }
  }, [page, article], {
    immediate: true
  });
      ...
      <div className="h-full text-justify">
        ...
        {/* 任务完成按钮 */}
        {
          ...
          <div className="w-full flex items-center justify-center mt-20">
            <button className="w-[80%] h-12 rounded-md text-3xl border-black border-2 text-black
                  active:bg-black active:text-white"
                    onClick={() => {
                      const part = article.split(':');
                      studentFinishTask(part[0], part[1])
                        .then(() => {
                          toast.info('任务已完成');
                          navigate(-1);
                        }).catch((error: Error) => {
