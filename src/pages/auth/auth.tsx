							...
							<form className={`flex flex-col gap-4 items-center`}
									...
									onChange={(value) => setUserData({ ...userData, password: value })} />
								{
									...
									<>
										...
										<Radio label="等级" value={userData.level} options={levelOptions}
													 onChange={(value) => setUserData({ ...userData, level: value })} />
										<Radio label="角色" value={userData.role} options={roleOptions}
											onChange={(value) => setUserData({ ...userData, role: value })} />
										<Input label="学校" type="text" required={true} value={userData.school}
													 placeholder="请输入你所在的学校名称" onChange={(value) => setUserData({ ...userData, school: value })} />
										<Input label="姓名" type="text" required={true} value={userData.name}
